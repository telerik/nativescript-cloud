...
export class HttpClient implements Server.IHttpClient {
	...
	private async httpRequestCore(options: any, proxySettings?: IProxySettings): Promise<Server.IResponse> {
		...
		if (_.includes([301, 302, 303, 307, 308], res.response.statusCode)) {
			if (res.response.statusCode === HttpStatusCodes.SEE_OTHER) {
				unmodifiedOptions.method = "GET";
			}

			this.$logger.trace("Begin redirected to %s", res.headers.location);
			unmodifiedOptions.url = res.headers.location;
